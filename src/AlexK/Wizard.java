...
public class Wizard implements MagicSource, Trader, MagicEffectRealization {
	...
	public Wizard(String name, MagicLevel level, int basicHP, int HP, int basicMP, int MP, int money,
		...
		if (name == null || name.isEmpty()) throw new IllegalArgumentException("Empty names are forbidden");
		if (level == null) throw new IllegalArgumentException("Level is not allowed to be null");
		if (basicHP < 0) throw new IllegalArgumentException("Basic health is not allowed to be negative");
		if (HP < 0) throw new IllegalArgumentException("Health is not allowed to be negative");
		if (basicMP < level.toMana()) throw new IllegalArgumentException("Basic mana is not allowed to be negative");
		if (MP < 0) throw new IllegalArgumentException("Mana is not allowed to be negative");
		if (money < 0) throw new IllegalArgumentException("Amount of money is not allowed to be negative");
		if (knownSpells == null) throw new IllegalArgumentException("Known spells set is not allowed to be null");
		if (protectedFrom == null) throw new IllegalArgumentException("Protected from set is not allowed to be null");
		if (carryingCapacity < 0) throw new IllegalArgumentException("Carrying capacity is not allowed to be negative");
		if (inventory == null) throw new IllegalArgumentException("Inventory set is not allowed to be null");
		this.name = name;
		this.level = level;
	...
	public boolean learn(Spell s) {
		if (s == null) throw new IllegalArgumentException("Spell is not allowed to be null");
	...
	public boolean castSpell(Spell s, MagicEffectRealization target) {
		if (s == null || target == null) throw new IllegalArgumentException("Spell/Target are not allowed to be null");
	...
	public boolean castRandomSpell(MagicEffectRealization target) {
		...
		for (Spell s: knownSpells) {
			if (i == randomIndex) return castSpell(s, target);
			i++;
		}
		return false;
	...
	public boolean useItem(Tradeable item, MagicEffectRealization target) {
		if (item == null || target == null) throw new IllegalArgumentException("Item/Target are not allowed to be null");
		if (isDead() || !possesses(item)) return false;
		item.useOn(target);
		return true;
	...
	public boolean useRandomItem(MagicEffectRealization target) {
		...
		for (Tradeable t: inventory) {
			if (i == randomIndex) return useItem(t, target);
			i++;
		}
		return false;
	}
	...
	public boolean sellRandomItem(Trader target) {
		...
		int i = 0;
		for (Tradeable t: inventory) {
			if (i == randomIndex) return sellItem(t, target);
			i++;
		}
		return false;
	}
	...
	public String toString() {
		...
		+ "): " + Integer.toString(HP) 
		+ "/" + Integer.toString(basicHP) + " " + Integer.toString(MP) + "/" + Integer.toString(basicMP) + "; " +
		Integer.toString(money) + (money == 1 ? " Knut": " Knuts") + "; knows " + knownSpells.toString() + 
	...
	public boolean provideMana(MagicLevel levelNeeded, int manaAmount) {
		if (levelNeeded == null) throw new IllegalArgumentException("LevelNeeded is not allowed to be null");
		if (manaAmount < 0) throw new IllegalArgumentException("Mana is not allowed to be negative");
		if (isDead()) return false;
		if (level.toMana() < levelNeeded.toMana()) return false;
	...
	public boolean addToInventory(Tradeable item) {
		if (item == null) throw new IllegalArgumentException("Item is not allowed to be null");
	...
	public boolean steal(Trader thief) {
		if (thief == null) throw new IllegalArgumentException("Thief is not allowed to be null");
	...
	public boolean loot(Trader looter) {
		if (looter == null) throw new IllegalArgumentException("Looter is not allowed to be null");
		if (!looter.canLoot()) return false;
		if (!isLootable()) return false;
		boolean smth_looted = false;
		Set <Tradeable> toBeRemoved = new HashSet<>(); 
		for (Tradeable el: inventory) {
			if (looter.addToInventory(el)) smth_looted = true;
			toBeRemoved.add(el);
		}
		for (Tradeable el: toBeRemoved)
			removeFromInventory(el);
		return smth_looted;
	}
	...
	public void takeDamage(int amount) {
		if (amount < 0) throw new IllegalArgumentException("Amount can't be neagtive");
	...
	public void takeDamagePercent(int percentage) {
		...
		takeDamage((int)(basicHP * (percentage/100.0)));
	}
	...
	public void weakenMagicPercent(int percentage) {
		if (percentage < 0 || percentage > 100) 
            throw new IllegalArgumentException("Percentage must be between 0 and 100");
		weakenMagic((int)(basicMP * (percentage/100.0)));
	}
	...
	public void healPercent(int percentage) {
		if (percentage < 0 || percentage > 100) throw new IllegalArgumentException("Perecntage must be in [0,100]");
		heal((int)(basicHP * percentage/100.0));
	}
	...
	public void enforceMagicPercent(int percentage) {
		if (percentage < 0 || percentage > 100)throw new IllegalArgumentException("Perecntage must be in [0,100]");
		enforceMagic((int)(basicMP *percentage/100.0));
	}
	...
	public boolean isProtected(Spell s) {
		...
		if (!(s instanceof AttackingSpell)) return false;
