...
public interface Tradeable {
     ...
     * @param to object is transferred to to's inventory
     * @return true, if transfer succeeds, false otherwise
     */
	private boolean transfer(Trader from, Trader to) {
		if (from == null || to == null)
			throw new IllegalArgumentException("Trader values are not allowed to be null");
		boolean remove = from.removeFromInventory(this);
		boolean add = to.addToInventory(this);
		return remove && add;
	}
	...
	default boolean give(Trader giver, Trader taker) {
		if (giver == null || taker == null || giver == taker)
			throw new IllegalArgumentException("Invalid taker or/and giver object(s) was/were provided");
		if (!giver.possesses(this) || !taker.hasCapacity(getWeight())) return false;
	...
	default boolean purchase(Trader seller, Trader buyer) {
		if (seller == null || buyer == null || seller == buyer)
			throw new IllegalArgumentException("Invalid taker or/and giver object(s) was/were provided");
		if (!seller.possesses(this) || !buyer.hasCapacity(getWeight()) || !buyer.canAfford(getPrice())) return false;
		boolean trans_res = false;
		if (buyer.pay(getPrice()) && seller.earn(getPrice())) { 
			trans_res = transfer(seller, buyer);
		}
		return trans_res;
