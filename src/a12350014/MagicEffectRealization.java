...
public interface MagicEffectRealization {
	...
	default void takeDamage(int amount) {
		if (amount < 0) throw new IllegalArgumentException("Amount of damage must be non-begative");
	...
	default void takeDamagePercent(int percentage) {
		if (percentage < 0 || percentage > 100) throw new IllegalArgumentException("Percentage value must be between 0 and 100");
	...
	default boolean isProtected(Spell s) {
		if (s == null) throw new IllegalArgumentException("Invalid spell was provided");
		return false;
	}
	...
	default void setProtection(Set<AttackingSpell> attacks) {
		if (attacks == null) throw new IllegalArgumentException("Invalid attacks set was provided");
	}
